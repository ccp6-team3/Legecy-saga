...
app.get("/reviewsMovie", async (req, res) => {
	...
	await getReviewsForMovie(movieID).then((data) => {
		data.forEach((review) => {
			let userReview = {};
			if (review.userName !== "") {
				userReview.author = review.userName;
			} else {
...
app.get("/searchMovies/:searchName", async (req, res) => {
	let searchMoviesArray = [];
	//DELETE AFTER TESTING
	console.log(req.get("Filter"));
	let SEARCH_KEY = req.params.searchName;
	fetch(
...
//Prototype for search bar
app.get("/searchTV/:searchName", (req, res) => {
	let searchTVArray = [];
	//DELETE AFTER TESTING
	let SEARCH_KEY = req.params.searchName;
	fetch(
